...
if (!$totalClientsResult) {
    die("Error fetching total clients: " . $conn->error);
}
...
if (!$loanRequestsResult) {
    die("Error fetching total loan requests: " . $conn->error);
}
$totalLoanRequests = $loanRequestsResult->fetch_assoc()['total_loan_requests'];

$totalLoanedQuery = "SELECT COALESCE(SUM(loan_amount), 0) AS total_loaned FROM loan_requests";
$totalLoanedResult = $conn->query($totalLoanedQuery);
if (!$totalLoanedResult) {
    die("Error fetching total loaned amount: " . $conn->error);
}
$totalLoaned = $totalLoanedResult->fetch_assoc()['total_loaned'];

// Define goals for progress bars
$totalClientsGoal = 100; // Example: Target 100 clients
$loanRequestsGoal = 500; // Example: Target 500 loan requests
$totalLoanedGoal = 1000000; // Example: Target 1,000,000 FCFA loaned

// Calculate progress percentages
$clientsProgress = ($totalClientsGoal > 0) ? ($totalClients / $totalClientsGoal) * 100 : 0;
$loanRequestsProgress = ($loanRequestsGoal > 0) ? ($totalLoanRequests / $loanRequestsGoal) * 100 : 0;
$totalLoanedProgress = ($totalLoanedGoal > 0) ? ($totalLoaned / $totalLoanedGoal) * 100 : 0;
?>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="stat-card">
                    ...
                    <h3>Total Loaned</h3>
                    <p class="value"><?= number_format($totalLoaned, 0) ?> FCFA</p>
